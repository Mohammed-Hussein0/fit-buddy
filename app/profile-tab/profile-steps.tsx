...
export default function ProfileStepsSection({ embedded = false }: ProfileStepsSectionProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const req = isGranted(perm.status) ? perm : await Pedometer.requestPermissionsAsync();
        if (cancelled) return;
      ...
      <View style={wrapStyle}>
        ...
        <View style={styles.card}>
          <View style={styles.row}>
            <Ionicons name="footsteps" size={22} color="#000" />
  ...
  if (loadState === 'unavailable') {
    return (
      <View style={wrapStyle}>
        ...
        <View style={styles.card}>
          <View style={styles.row}>
            <Ionicons name="footsteps-outline" size={22} color="#999" />
            <Text style={styles.title}>Steps today</Text>
          </View>
    ...
    return (
      <View style={wrapStyle}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.muted}>
            Allow Physical activity for this app (Android 10+). On Samsung: Settings → Apps → Fit Buddy
            → Permissions → Physical activity. Location is not required for steps.
          </Text>
          {Platform.OS === 'android' && Constants.appOwnership === 'expo' && (
            <Text style={styles.hint}>
              You are in Expo Go: enable Physical activity for the Expo Go app (Settings → Apps → Expo
              Go → Permissions), not only Fit Buddy.
            </Text>
          )}
    ...
    return (
      <View style={wrapStyle}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.muted}>Could not read steps. Try again.</Text>
          <TouchableOpacity
  ...
  return (
    <View style={wrapStyle}>
      ...
      <View style={styles.card}>
        ...
        {Platform.OS === 'android' && (
          <Text style={styles.hint}>Updates while this screen is open (Android).</Text>
        )}
      </View>
    </View>
  );
